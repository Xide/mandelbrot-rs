    ...
    for cap in re.captures_iter(fmt) {
        ...
        return cap[1]
            ...
            .and_then(|re| {
                cap[2]
                    .parse::<f64>()
                    .and_then(|im| Ok(Some(Complex64::new(re, im))))
            })
            .unwrap_or(None);
    }
    None
...
fn main() {
    ...
    let dims = parse_dimensions(matches.value_of("resolution").unwrap_or("1920x1080"))
    ...
    let seed = parse_seed(matches.value_of("seed").unwrap_or("(-0.25, 0.0)"))
