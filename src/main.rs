...
fn main() {
    ...
    let seed = parse_seed(matches.value_of("seed").unwrap_or("(-2.5, 1.25)")).expect("Seed format: '(re, im)'");
    let camera_width = matches.value_of("camera_width").unwrap_or("4.5").parse::<f64>().expect("Expected a float number as camera width");
